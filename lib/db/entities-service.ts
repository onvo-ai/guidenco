...
export async function listEntities(organizationId: string) {
  const [documentRows, assetRows, videoRows, articleRows, socialPostRows] =
    await Promise.all([
      db
        .select()
        .from(documents)
      ...
      db
        .select()
        .from(blogArticles)
        .where(eq(blogArticles.organizationId, organizationId))
  ...
  const documentsWithStats = await Promise.all(
    documentRows.map(async (document) => {
      ...
      const currentVersionRows = await db
        .select({
          html: documentVersions.html,
          thumbnail: documentVersions.thumbnail,
          width: documentVersions.width,
          height: documentVersions.height,
        })
        .from(documentVersions)
        .where(eq(documentVersions.documentId, document.id))
  ...
  const assetsWithStats = await Promise.all(
    assetRows.map(async (asset) => {
      ...
      const currentVersionRows = await db
        .select({
          svgContent: assetVersions.svgContent,
  ...
  const articlesWithStats = await Promise.all(
    articleRows.map(async (article) => {
      ...
      const currentVersionRows = await db
        .select({
          content: blogArticleVersions.content,
          bannerImage: blogArticleVersions.bannerImage,
          tags: blogArticleVersions.tags,
        })
        .from(blogArticleVersions)
        .where(eq(blogArticleVersions.articleId, article.id))
  ...
  const socialPostsWithStats = await Promise.all(
    socialPostRows.map(async (post) => {
      const [{ value: versionCount }] = await db
        .select({ value: count() })
      ...
      const currentVersionRows = await db
        .select({
          content: socialPostVersions.content,
          hashtags: socialPostVersions.hashtags,
          mediaUrl: socialPostVersions.mediaUrl,
          mediaType: socialPostVersions.mediaType,
        })
        .from(socialPostVersions)
        .where(eq(socialPostVersions.postId, post.id))
...
export async function deleteEntity(id: string, organizationId: string) {
  ...
  const deletedVideo = await db
    .delete(videos)
    .where(and(eq(videos.id, id), eq(videos.organizationId, organizationId)))
  ...
  const deletedArticle = await db
    ...
    .where(
      and(
        eq(blogArticles.id, id),
        eq(blogArticles.organizationId, organizationId),
      ),
    )
  ...
  await db
    ...
    .where(
      and(
        ...
        eq(socialPosts.organizationId, organizationId),
      ),
    );
}
...
export async function updateDocumentSettings(
  ...
  if (updated && width !== undefined && height !== undefined) {
    await db
      ...
      .where(
        and(
          ...
          eq(documentVersions.version, updated.currentVersion),
        ),
      );
  }
...
export async function getDocumentMessages(documentVersionId: string) {
  return await db
    ...
    .where(eq(documentChatMessages.documentVersionId, documentVersionId))
    .orderBy(documentChatMessages.createdAt);
}

export async function saveDocumentMessage(
  ...
  await db
    ...
    .values({ documentVersionId, role, content });
}

export async function createPendingDocumentVersion(
  documentId: string,
  opts: { prompt?: string; parentVersionId?: string; model?: string } = {},
) {
  let [documentData] = await db
    .select()
    .from(documents)
...
export async function updateDocumentVersionContent(
  versionId: string,
  data: {
    html: string;
    width: number;
  ...
  await db
    ...
    .set({
      ...
      height: data.height,
      googleFonts:
        data.googleFonts && data.googleFonts.length > 0
          ? data.googleFonts
          : null,
...
export async function getAssetById(assetId: string) {
  ...
  const mappedVersions = versions.map((v) => ({
    ...
    parentVersionId: v.parentVersionId ?? undefined,
    model: v.model ?? undefined,
    tokenCount: v.tokenCount ?? undefined,
    creditCount: v.creditCount ?? undefined,
  }));

  return {
    ...asset,
    svgContent: mappedVersions[asset.currentVersion ?? 0]?.svgContent,
    versions: mappedVersions,
  };
}
...
export async function upsertAsset(
  ...
  const shouldCreateVersion =
    data.createVersion ?? data.svgContent !== undefined;
...
export async function updateVideoVersion(
  versionId: string,
  data: { videoUrl?: string; status?: string },
) {
...
export async function upsertVideo(
  ...
  const shouldCreateVersion =
    data.createVersion ??
    Boolean(
      data.remotionCode !== undefined ||
      data.title !== undefined ||
      data.width !== undefined ||
      data.height !== undefined ||
      data.durationInFrames !== undefined ||
      data.fps !== undefined,
    );
...
export async function getVideoMessages(videoVersionId: string) {
  return await db
    ...
    .where(eq(videoChatMessages.videoVersionId, videoVersionId))
...
export async function getBlogArticleMessages(articleVersionId: string) {
  return await db
    .select()
    .from(blogArticleChatMessages)
    .where(eq(blogArticleChatMessages.articleVersionId, articleVersionId))
    .orderBy(blogArticleChatMessages.createdAt);
}

export async function saveBlogArticleMessage(
  articleVersionId: string,
  role: "user" | "assistant",
  content: any,
) {
  await db
    .insert(blogArticleChatMessages)
    .values({ articleVersionId, role, content });
}
...
export async function getSocialPostMessages(postVersionId: string) {
  return await db
    .select()
    .from(socialPostChatMessages)
    .where(eq(socialPostChatMessages.postVersionId, postVersionId))
    .orderBy(socialPostChatMessages.createdAt);
}

export async function saveSocialPostMessage(
  postVersionId: string,
  role: "user" | "assistant",
  content: any,
) {
  await db
    .insert(socialPostChatMessages)
...
export async function getExperimentById(experimentId: string) {
  ...
  const parameters = await db
    ...
    .where(eq(experimentParameters.experimentId, experiment.id));
...
export async function getExperimentsForEntity(entityId: string) {
  return await db
    .select()
    .from(experiments)
    .where(eq(experiments.entityId, entityId))
