...
This guide will help you run the Guidenco application using Docker and Docker Compose.
...
# Database URL
POSTGRES_URL=postgresql://guidenco:your_secure_password@postgres:5432/guidenco_db
...
```bash
docker compose exec postgres psql -U guidenco -d guidenco_db
```
...
```bash
# Build image
docker build -t your-registry/guidenco:1.0.0 .

# Push to registry
docker push your-registry/guidenco:1.0.0
```
...
```env
NODE_ENV=production
POSTGRES_URL=postgresql://user:password@prod-db-host:5432/guidenco_db
NEXT_PUBLIC_APP_URL=https://guidenco.example.com
