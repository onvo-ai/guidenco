#!/usr/bin/env bash
# Guidenco Pi installer — curl -fsSL https://guidenco.app/install.sh | bash
set -euo pipefail
...
# ── 1. Preflight ──────────────────────────────────────────────────────────────
[[ "$(uname -s)" == "Linux" ]] || error "This script requires Linux (Raspberry Pi OS)"
...
# ── 6. Environment file (fresh install) ───────────────────────────────────────
DEVICE_ID=$(python3 -c "import uuid; print(str(uuid.uuid4()))")
sudo mkdir -p /etc/guidenco
{ echo "DEVICE_ID=$DEVICE_ID"; echo "CLOUD_URL=$CLOUD_URL"; } | sudo tee "$ENV_FILE" > /dev/null
