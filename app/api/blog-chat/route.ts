...
import { z } from 'zod';
import { saveBlogArticleMessage, upsertBlogArticle, getBlogArticleById, getBlogArticleVersionById } from '@/lib/db/entities-service';
...
export async function POST(req: Request) {
  try {
    ...
    if (lastUserMessage?.role === 'user') {
      await saveBlogArticleMessage(articleId, 'user', lastUserMessage.parts || []);
    }
    ...
    const configuredModel = (process.env.BLOG_MODEL || process.env.OPENROUTER_MODEL || '').trim();
...
CRITICAL: Call saveArticle EXACTLY ONCE per user request. Never call it twice.`,
      tools: {
        saveArticle: tool({
          ...
          inputSchema: z.object({
            ...
            bannerImage: z.string().optional().describe('URL of the banner image to display at the top of the article'),
            tags: z.array(z.string()).optional().describe('List of topic tags for the article (e.g., ["react", "performance", "web"])'),
          }),
          execute: async ({ content, title, bannerImage, tags }: { content: string; title?: string; bannerImage?: string; tags?: string[] }) => {
            try {
              await upsertBlogArticle(articleId, { content, title, bannerImage, tags, createVersion: true, prompt: userPrompt, parentVersionId });
        ...
        getArticle: tool({
          ...
          inputSchema: z.object({}),
          execute: async () => {
            const content = parentContent ?? (await getBlogArticleById(articleId))?.content;
        ...
        searchAssets: tool({
          description: 'Search the user\'s generated SVG assets by title. Returns a list of assets with their URLs that can be embedded in the article.',
          inputSchema: z.object({
          ...
          execute: async ({ query }: { query?: string }) => {
            try {
              ...
              if (query) {
                ...
                ).orderBy(desc(assets.updatedAt)).limit(20);
                rows = rows.filter(a => a.title.toLowerCase().includes(query.toLowerCase()));
              } else {
                rows = await db.select().from(assets).where(eq(assets.organizationId, organizationId)).orderBy(desc(assets.updatedAt)).limit(20);
              }
              return {
                success: true,
                assets: rows.map(a => ({
        ...
        searchVideos: tool({
          description: 'Search the user\'s generated videos by title. Returns a list of rendered videos with their URLs.',
          inputSchema: z.object({
        ...
        searchBrandAssets: tool({
          description: 'Search the team\'s brand asset warehouse (uploaded images, logos, videos, PDFs) by title or description.',
          inputSchema: z.object({
        ...
        searchStockPhotos: tool({
          ...
          inputSchema: z.object({
            query: z.string().describe('Search keywords describing the image needed (e.g., "technology workspace", "nature landscape", "business meeting")'),
      ...
      onStepFinish: async ({ text, toolCalls, toolResults, usage }) => {
        ...
        try {
          ...
          if (parts.length > 0) await saveBlogArticleMessage(articleId, 'assistant', parts);
        } catch (e) {
    ...
    const message = error instanceof Error ? error.message : 'Unknown blog chat error';
