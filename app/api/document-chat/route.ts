...
import { z } from 'zod';
import { createOrUpdateDocument, getDocumentById, saveDocumentMessage, createPendingDocumentVersion, updateDocumentVersionContent, updateDocumentVersionStatus } from '@/lib/db/entities-service';
...
async function getUserTeamAndAgentSettings(userId: string) {
  ...
  return { organizationId, designGuidelines: agentData[0]?.designGuidelines || '' };
}
...
export async function POST(req: Request) {
  ...
  let activeVersionId: string | undefined = parentVersionId ?? currentDoc?.versions[currentDoc.currentVersion]?.id;
  ...
  if (lastUserMessage?.role === 'user' && activeVersionId) {
    await saveDocumentMessage(activeVersionId, 'user', lastUserMessage.parts || []).catch(() => { });
  }
  ...
  if (parentVersionId) {
    const parentVer = currentDoc?.versions.find(v => v.id === parentVersionId);
    if (parentVer) {
    ...
    for (const part of msg.parts || []) {
      if (part.type === 'text') {
        ...
        if (mimeType === 'image/svg+xml') {
          content.push({ type: 'text', text: part.fileUrl ? `[SVG reference provided. Use this asset URL if needed: ${part.fileUrl}]` : '[SVG reference provided in chat context.]' });
        ...
        if (part.fileUrl) {
          content.push({ type: 'text', text: `[Image uploaded to Design Warehouse. Permanent URL: ${part.fileUrl} — use this URL in <img src="..."> tags when embedding this image in document.]` });
        }
      }
    }
...
CRITICAL: ALWAYS provide a text response after using tools.`,
    tools: {
      ...
      writeHTML: tool({
        description: 'Write HTML with Tailwind CSS classes, FontAwesome icons, and optional Google Fonts. This REPLACES the current page with a new version.',
        inputSchema: z.object({
          html: z.string().describe('Complete HTML with Tailwind CSS classes.'),
          googleFonts: z.array(z.string()).optional().describe('Array of Google Font family names to load'),
          pageIndex: z.number().int().min(0).optional().describe('If provided, updates only this page (0-based index)'),
        }),
        execute: async ({ html, googleFonts, pageIndex }: { html: string; googleFonts?: string[]; pageIndex?: number }) => {
          try {
            ...
            if (!dimensions) {
              if (existingDocument) {
                ...
                return { success: false, error: 'No document exists. Please create a document first using createDocument.' };
              }
            }

            const sourceHtml = parentHtml !== undefined
              ? parentHtml
              : existingDocument?.versions[existingDocument.currentVersion]?.html || '';
            const sourceFonts = parentGoogleFonts.length > 0
              ? parentGoogleFonts
              : existingDocument?.versions[existingDocument.currentVersion]?.googleFonts || [];
            ...
            if (pendingVersionId) {
              ...
              return { success: true, message: 'HTML updated successfully (Version 1)', version: 0, totalVersions: 1, width: dimensions.width, height: dimensions.height, pageCount: storedPages.length, pageIndex: resolvedPageIndex };
            }

            const { currentVersion, totalVersions, newVersionId: wvId } = await createOrUpdateDocument(
              documentId, existingDocument?.title ?? 'Untitled Document',
              dimensions.width, dimensions.height, htmlToStore, fontsToUse, userPrompt, parentVersionId, modelId
            );
            ...
            return { success: true, message, version: currentVersion, totalVersions, width: dimensions.width, height: dimensions.height, pageCount: storedPages.length, pageIndex: resolvedPageIndex };
          } catch (error: any) {
            console.error('Error in writeHTML:', error);
            return { success: false, error: `Failed to write HTML: ${error.message}` };
          }
        },
      }),
      editHTML: tool({
        description: 'Make surgical edits to existing HTML by finding and replacing specific parts.',
        inputSchema: z.object({
          findString: z.string().describe('The exact HTML string to find and replace.'),
          replaceString: z.string().describe('The new HTML string to replace it with.'),
          pageIndex: z.number().int().min(0).optional().describe('If provided, edits only this page (0-based index).'),
        }),
        execute: async ({ findString, replaceString, pageIndex }: { findString: string; replaceString: string; pageIndex?: number }) => {
          try {
            ...
            const targetPageIndex = clampPageIndex(typeof pageIndex === 'number' ? pageIndex : 0, pages.length);
            const pageHTML = pages[targetPageIndex] || '';

            if (!pageHTML.includes(findString)) return { success: false, error: `Could not find the specified string in page ${targetPageIndex + 1}.` };
            const occurrences = pageHTML.split(findString).length - 1;
            if (occurrences > 1) return { success: false, error: `The findString appears ${occurrences} times. Please provide a more specific string.` };

            pages[targetPageIndex] = pageHTML.replace(findString, replaceString);
            const htmlToStore = joinPages(pages);
            const { currentVersion, totalVersions, newVersionId: evId } = await createOrUpdateDocument(
              documentId, document.title, document.width, document.height, htmlToStore, sourceFonts, userPrompt, parentVersionId, modelId
            );
            if (evId) latestVersionId = evId;
            return { success: true, message: `HTML edited successfully (Version ${currentVersion + 1})`, version: currentVersion, totalVersions, pageCount: pages.length, pageIndex: targetPageIndex };
          } catch (error: any) {
            return { success: false, error: `Failed to edit HTML: ${error.message}` };
          }
        },
      }),
      writePagesHTML: tool({
        description: 'Write ALL pages of a multi-page document in a SINGLE version.',
        inputSchema: z.object({
          pages: z.array(z.string()).min(1).max(50).describe('Array of page HTML strings, in order.'),
          googleFonts: z.array(z.string()).optional().describe('Array of Google Font family names to load'),
        }),
        execute: async ({ pages, googleFonts }: { pages: string[]; googleFonts?: string[] }) => {
          try {
            let dimensions = tempDimensions.get(documentId);
            const requestedPageCount = tempPageCounts.get(documentId);
            let existingDocument = await getDocumentById(documentId);
            if (!dimensions) {
              if (existingDocument) { dimensions = { width: existingDocument.width, height: existingDocument.height }; }
              else return { success: false, error: 'No document exists. Please create one first using createDocument.' };
            }
            ...
            if (pendingVersionId) {
              ...
              latestVersionId = pendingVersionId;
              pendingVersionId = undefined;
              return { success: true, message: 'HTML updated successfully (Version 1)', version: 0, totalVersions: 1, pageCount: normalizedPages.length };
            }

            const { currentVersion, totalVersions, newVersionId: wpId } = await createOrUpdateDocument(
              documentId, existingDocument?.title ?? 'Untitled Document',
              dimensions.width, dimensions.height, htmlToStore, googleFonts || [], userPrompt, parentVersionId, modelId
            );
            if (wpId) latestVersionId = wpId;
            return { success: true, message: `HTML updated successfully (Version ${currentVersion + 1})`, version: currentVersion, totalVersions, pageCount: normalizedPages.length };
          } catch (error: any) {
            return { success: false, error: `Failed to write pages HTML: ${error.message}` };
          }
        },
      }),
      createPage: tool({
        ...
        inputSchema: z.object({
          afterPageIndex: z.number().int().min(-1).optional().describe('Insert after this 0-based index. -1 = beginning. Default = append.'),
        }),
        execute: async ({ afterPageIndex }: { afterPageIndex?: number }) => {
          try {
            ...
            const { currentVersion, totalVersions, newVersionId: cpId } = await createOrUpdateDocument(
              documentId, document.title, document.width, document.height, joinPages(nextPages), sourceFonts, userPrompt, parentVersionId, modelId
            );
            if (cpId) latestVersionId = cpId;
            return { success: true, message: `Page created (Version ${currentVersion + 1})`, pageCount: nextPages.length, pageIndex: insertAt, version: currentVersion, totalVersions };
          } catch (error: any) {
      ...
      deletePage: tool({
        description: 'Delete a page from the current document by 0-based pageIndex.',
        inputSchema: z.object({
        ...
        execute: async ({ pageIndex }: { pageIndex: number }) => {
          try {
            ...
            const sourceFonts = parentGoogleFonts.length > 0 ? parentGoogleFonts : document.versions[document.currentVersion]?.googleFonts || [];
            const pages = splitPages(sourceHtml);
            if (pages.length <= 1) return { success: false, error: 'Cannot delete the last remaining page.' };
            if (pageIndex < 0 || pageIndex >= pages.length) return { success: false, error: `Invalid pageIndex.` };
            const nextPages = pages.filter((_, idx) => idx !== pageIndex);
            const { currentVersion, totalVersions, newVersionId: dpId } = await createOrUpdateDocument(
              documentId, document.title, document.width, document.height, joinPages(nextPages), sourceFonts, userPrompt, parentVersionId, modelId
            );
            if (dpId) latestVersionId = dpId;
            return { success: true, message: `Page deleted (Version ${currentVersion + 1})`, pageCount: nextPages.length, deletedPageIndex: pageIndex, version: currentVersion, totalVersions };
          } catch (error: any) {
      ...
      getDocumentState: tool({
        description: 'Get the current rendered state of the document as an image. Use this to visually inspect what has been created so far.',
        inputSchema: z.object({
        ...
        execute: async ({ pageIndex }: { pageIndex?: number }) => {
          try {
            ...
            const resolvedPageIndex = clampPageIndex(typeof pageIndex === 'number' ? pageIndex : 0, pages.length);
            ...
            try {
              const renderResponse = await fetch(`${process.env.NEXT_PUBLIC_APP_URL}/api/render`, {
                ...
                body: JSON.stringify({ template: pageHTML, width: document.width, height: document.height, format: 'base64', googleFonts: currentFonts }),
              });
              if (renderResponse.ok) {
                const renderData = await renderResponse.json();
                imageData = renderData.image;
                if (imageData) { try { imageData = await resizeImage(imageData); } catch (e) { console.error('Failed to resize image:', e); } }
              }
            } catch (renderError) { console.error('Error rendering image:', renderError); }
            return { success: true, width: document.width, height: document.height, image: imageData, version: document.currentVersion, totalVersions: document.versions.length, pageCount: pages.length, pageIndex: resolvedPageIndex };
          } catch (error: any) {
            return { success: false, error: `Error getting document state: ${error.message}` };
          }
        },
      }),
      searchImage: tool({
        ...
        execute: async ({ query, count = 1 }: { query: string; count?: number }) => {
          try {
            ...
            if (!response.ok) return { success: false, error: 'Failed to search images' };
            const data = await response.json();
            return {
              success: true, query,
              images: data.results.map((img: any) => ({ url: img.urls.regular, thumbnail: img.urls.small, width: img.width, height: img.height, description: img.description || img.alt_description, photographer: img.user.name })),
            };
          } catch { return { success: false, error: 'Error searching for images' }; }
        },
      }),
      listAssets: tool({
        ...
        execute: async () => {
          try {
            ...
            return { success: true, assets: teamAssets.map(a => ({ id: a.id, title: a.title, description: a.description, fileUrl: a.fileUrl, mimeType: a.mimeType })) };
          } catch { return { success: false, error: 'Failed to list assets', assets: [] }; }
        },
      }),
      inspectAsset: tool({
        description: 'Inspect a specific asset from the Design Warehouse. Returns a rendered image.',
        inputSchema: z.object({ assetId: z.string().describe('The ID of the asset to inspect') }),
        execute: async ({ assetId }: { assetId: string }) => {
          try {
            ...
            return { success: true, title: a.title, description: a.description, mimeType: a.mimeType, fileUrl: a.fileUrl, proxyUrl: `/api/assets/image?id=${a.id}`, image: `data:${a.mimeType};base64,${base64}` };
          } catch { return { success: false, error: 'Failed to inspect asset' }; }
        },
      }),
    },
    onFinish: async () => {
    ...
    onStepFinish: async ({ text, toolCalls, toolResults, usage }) => {
      ...
      try {
        ...
        if (parts.length > 0 && latestVersionId) await saveDocumentMessage(latestVersionId, 'assistant', parts);
      } catch (e) {
