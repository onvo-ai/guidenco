...
# --- Mouse: HID absolute pointer with wheel (no Report ID) ---
# subclass=0/protocol=0 because boot mouse is relative; we need absolute coords.
# The wheel is required, not optional: VNC signals scrolling as presses of
# buttons 4-7, and without a wheel axis there is nowhere to send them.
_w 0                     "$GADGET/functions/hid.usb1/protocol"
_w 0                     "$GADGET/functions/hid.usb1/subclass"
_w 6                     "$GADGET/functions/hid.usb1/report_length"

python3 - "$GADGET/functions/hid.usb1/report_desc" <<'PYEOF'
import sys
# Absolute mouse with wheel. Report = 6 bytes:
#   buttons(1, low 3 bits) + X(2 LE, 0..32767) + Y(2 LE, 0..32767) + wheel(1 signed)
desc = bytes([
    0x05,0x01,        # Usage Page (Generic Desktop)
    ...
    # Clear the physical range inherited from X/Y — those are absolute 0..32767,
    # the wheel is a relative delta and must not carry that mapping over.
    0x35,0x00,0x45,0x00,
    0x75,0x08,0x95,0x01,
    0x81,0x06,        #     Wheel, relative (8-bit signed)
    0xc0,
    0xc0,
])
open(sys.argv[1], 'wb').write(desc)
