...
export function SignInForm({ onSuccess }: { onSuccess?: () => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Clear the oidc_login_prompt cookie so better-auth doesn't auto-redirect
      // to the OAuth callback via fetch (which causes a cross-origin CORS error).
      // The onSuccess handler navigates there via window.location.href instead.
      if (typeof document !== 'undefined') {
        document.cookie = 'oidc_login_prompt=; Max-Age=0; Path=/; SameSite=Lax';
      }
