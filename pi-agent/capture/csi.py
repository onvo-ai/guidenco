...
from config import VIDEO_DEV, CAPTURE_W, CAPTURE_H, STREAM_FPS
...
class CsiBackend(CaptureBackend):
    # v4l2-ctl skips the partial frame 0 itself; ffmpeg sees clean frames from
    # frame 1, so no JPEG-level warmup skipping is needed.
    warmup_frames = 0
    ...
    def open(self) -> tuple[subprocess.Popen, list[subprocess.Popen]]:
        ...
        return ffmpeg_proc, [ffmpeg_proc, v4l2_proc]
